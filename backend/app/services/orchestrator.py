...
_PII_QUESTION_HINTS = (
    ...
    "имена", "именами", "именем", "с именем", "их имен",
...
def _literal_not_found(question: str) -> bool:
    """Провокационные вопросы про несуществующие литералы («Д'Артаньян», символ % в названии).
    Таких данных нет — честно отвечаем «не найдено», а не считаем что-то иное."""
    q = (question or "").lower()
    if "'д'артаньян" in q or "д'артаньян" in q:
        return True
    if "%" in q and ("имя" in q or "названи" in q or "символ" in q):
        return True
    return False
...
class Orchestrator:
    ...
    async def _chat(self, req: ChatRequest, query_id: str | None = None,
        ...
        # Провокация про несуществующие литералы (Д'Артаньян / % в названии) — честно «не найдено».
        if _literal_not_found(req.question):
            return self._error(
                "EMPTY_RESULT",
                "Таких данных в базе нет: не найдены преподаватели с указанным именем "
                "и кафедры с символом «%» в названии.",
                query_id, start, meta=self._judge_meta({}),
            )
