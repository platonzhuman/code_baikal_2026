...
VALUE_MAP = (
    ...
    "· ⭐ должник/должники/должен/имеет долг/долг/долженник → студент с ХОТЯ БЫ ОДНОЙ записью "
    "enrollments.passed=false. Считай COUNT(DISTINCT e.student_id). НЕ «платная форма(paid)», "
    "НЕ «зачислен(enrolled)» — это ОШИБКА;\n"
    "· зачисленный студент/обучающийся → students.status='active'; зачисленный абитуриент → "
    "applicants.status='enrolled'; «поступил в X году» → students.enrolled_year=X;\n"
    "· выбыл/отселён/отчислен/вылетел → status='expelled'; академический отпуск/академ → academic_leave;\n"
    "· педагог/преподаватель/лектор → staff (post='преподаватель');\n"
    "· оценка/отметка/балл → enrollments.grade; сессия/успеваемость/экзамен/зачёт → enrollments;\n"
    "· пропуски/посещаемость → enrollments.attendance;\n"
    "· направление/специальность/специализация → programs; факультет/институт → faculties; "
    "кафедра/отделение → departments;\n"
    "· бюджет/бюджетник/казённый → source='budget'; платно/платник/коммерческий → source='paid';\n"
    "· очная/день → form='fulltime'; заочная/очно-заочная/дистанционка → form='parttime';\n"
    "· первокурсник/второкурсник/курс/поток → students.course;\n"
    "· зимняя/первая/осенняя сессия → '... fall'; летняя/весенняя/вторая сессия → '... spring';\n"
    ...
    "· «по математике/русскому» → applicants.ege_math / ege_rus; «с первой попытки» → attempt=1;\n"
    "· рейтинг/успеваемость/средний балл студента → students.gpa (или AVG(gpa));\n"
    "· «перевод/перевёлся/остался на N курс»/«второкурсник» → students.course=N (и status='active');\n"
    "· «год набора/набор 20XX» → students.enrolled_year=X (для абитуриентов — год из submitted_date);\n"
    "· «проходной/проходная (балл)» → programs.min_score_prev;\n"
    "· «количество пар/парность» → schedule.pair; «аудиторный фонд/вместимость» → rooms.capacity;\n"
